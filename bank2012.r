...
# rewrite CO2 emissions data by transferring 2010 (no data for 2012) data from original dataset
for(i in 4184:4429) {
  # where country code matches, copy 2010 data to applicable column
  if(bank[i,4] %in% bank2012[,2]) {
    bank2012[bank2012[,1]==bank[i,3],c(bank[i,2])] <- bank[i,c("X2010")]
  }
}
...
# output as csv file
write.csv(bank2012, file="bank2012.csv")
